Understanding pixelcraft
======================

As mentioned in intro section pixelcraft consists of two modules,
Video and Image, In this section we will go into details about 
each modules working. 

PixelCraft.video Module:
--------------------
...
recursively.
PixelCraft.video frame extraction feature first takes a video and divides a big video in smaller chunks of 
...
PixelCraft by following two separate modules.

Details about public  **compress_video** and **compress_videos_from_dir**
functions is listed in :ref:`PixelCraft.video_compressor`. 

PixelCraft.frame_extractor module
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
PixelCraft.frame_selector module
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

In PixelCraft.frame_selector module given list of frames
...
and edit in PixelCraft.config.ImageSelector properties. 
...
.. _PixelCraft.video_compressor:

PixelCraft.video_compressor module
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Apart from Frame extraction PixelCraft.video module can also do efficient video
compression. It is done by internal module called internally by PixelCraft.video_compressor
...
PixelCraft.video_compressor includes actual implementation of video compression using ffmpeg
library.
...
you can change default value by changing PixelCraft.config.Video.video_compression_codec
...
.. _PixelCraft.video_resize:

PixelCraft.video_resize module
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
As mentioned in home section since version 0.8.0 of PixelCraft we are extending
smart resize features to videos with the help of Google's Mediapipe project.
In simple terms video resize functionality in PixelCraft currently is a thin
...
for how to install and configure mediapipe to be used with pixelcraft. 
Right now following parameters are configurable using PixelCraft video module:
...
.. _PixelCraft.mediapipe_integration:

Mediapipe Autoflip Integration
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
PixelCraft uses python multiprocessing to create a pool of processes based on the number of cpu cores on machine, 
and launch processes to resize video on these cores. For example on a 12 core machine, if 4 videos needs
to be resized in parallel, PixelCraft will launch mediapipe autoflip pipeline for 4 videos on 4 cores.

Since mediapipe is installed outside of PixelCraft, mediaipie autoflip pipeline is launched using the 
python subprocess module. To ensure smooth run, PixelCraft needs build autorun directory path (where binaries are build)
and also the mediapipe models directory which containe tflite (tensorflow lite models for CPU). Internally, PixelCraft creates
a simlink to the models folder directory during its instance of execution. This allow
PixelCraft to access the models file required to run autoflip pipeline.
...
.. figure:: images/mediapipe_autoflip_integration.jpeg
         :width: 100%
         :align: center
         :alt: PixelCraft Mediapipe Autoflip Integration


PixelCraft provides an interface to configure autoflip. This enables PixelCraft to hide the complexity
of autoflip graphs configuration (.pbtxt file) and provide users will relevant configurations
in the form of python friendly dictionary. Behind the scenes, PixelCraft will create a temporary graph (.pbtxt)
file based on the configuration provided and use it to run the autoflip pipeline.

PixelCraft users can check **temp_pbtxt** directory when the pipeline is running to look at 
the mediapipe autoflip graph file. The folders gets deleted automatically when the pipeline
finishes its execution.

To check the list of configurable options, check :ref:`PixelCraft.video_resize`. 


PixelCraft.image Module:
---------------------
...
For a given input cropping dimension/final output image size, PixelCraft.image works
by first extracting all possible image crop given crop specification using 
pixelcraft.crop_extractor module, PixelCraft.crop_selector module then uses various filtering
and selection criteria to select best crops from list of image crops.
Let's read more about these two modules in details. 

PixelCraft.crop_extractor module
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
PixelCraft.crop_extractor module works by given a crop specification using a sliding
window approach it first calculates all possible crop see
**_get_all_possible_crops()** function inside PixelCraft.crop_extractor module.
...
PixelCraft.config.CropScorer module. 


PixelCraft.crop_selector module
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
After returning candidate crops from crop_extractor module PixelCraft.crop_selector
module first does further filtering using PixelCraft.image_filters filters.
...
Configurations related to these scoring rules could be edited in
PixelCraft.config.CropScorer, PixelCraft.config.EdgeFeature, PixelCraft.config.FaceFeature modules.
