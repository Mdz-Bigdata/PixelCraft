...
import errno
import ntpath
from PixelCraft.decorators import VideoDecorators
from PixelCraft.decorators import FileDecorators

from PixelCraft.frame_extractor import FrameExtractor
from PixelCraft.image_selector import ImageSelector
from PixelCraft.mediapipe import MediaPipeAutoFlip
import PixelCraft.config as config
from PixelCraft.video_compressor import VideoCompressor
import PixelCraft.helper_functions as helper
...
class Video(object):
    ...
    def compress_video(
        ...
        PixelCraft.config.Video.video_compression_codec
    ...
    def compress_videos_from_dir(
        ...
        output video sizes with same or better quality. Right now libx265 is not as widely compatible \
        on older versions of MacOS and Widows by default. If wider video compatibility is your goal \
        you should use libx264., you can change default value by changing PixelCraft.config.Video.video_compression_codec
        :type output_video_codec: str, optional
        :param out_dir_path: output folder path where you want output video to be saved, defaults to ""
        :type out_dir_path: str, optional
        :raises Exception: raises FileNotFoundError Exception if input video file not found, also exception is raised in case output video file path already exist and force_overwrite is not set to True.
